...
export const createProduct = async (req: Request, res: Response) => {
  try {
    ...
    if (!images || images.length === 0) {
      return response(res, 400, "images is required", null);
    }
    ...
    if (
      ...
      return response(res, 400, "UPI ID is required for payments", null);
    }
    ...
    if (
      ...
      return response(res, 400, "Bank details are required for payments", null);
    }
...
export const deleteProduct = async (req: Request, res: Response) => {
    try {
        ...
        if (!product) {
            return response(res, 404, "Product not found for this seller", null);
        }
        return response(res, 200, "Product deleted successfully", null);
    } catch (error) {
        console.error(error);
        response(res, 500, "Internal server error", null);
    }
...
export const getProductBySellerId = async (req: Request, res: Response) => {
    try {
        ...
        if(!sellerId){
            return response(res, 400, "Seller ID is required", null);
        }
        ...
        if(!products || products.length === 0) {
            return response(res, 404, "No products found for this seller", null);
        }
